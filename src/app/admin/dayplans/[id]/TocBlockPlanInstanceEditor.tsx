...
function inferDefaults(blockLabel: string | null) {
...
async function ensureDefaultTemplateForClass(supabase: ReturnType<typeof getSupabaseClient>, classId: string) {
  ...
  if (anyTpl) {
    ...
    if (!(anyTpl as any).is_active) {
      await supabase.from('class_toc_templates').update({ is_active: true, updated_at: new Date().toISOString() }).eq('id', (anyTpl as any).id);
    }
  ...
  const d = inferDefaults((klass as any).block_label ?? null);
