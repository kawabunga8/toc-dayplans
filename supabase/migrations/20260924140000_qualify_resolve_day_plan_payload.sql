-- Correct 20260924120000: it created resolve_day_plan_payload without a schema
-- prefix, so (as with get_public_classes, see ADR-0004) the definition landed
-- in rcs instead of public. Result: public.resolve_day_plan_payload — the one
-- the publish route calls — still attached students to every block, so the
-- next publish would have written names back into published_payload; and the
-- stray rcs copy was overwritten and re-granted to authenticated, undoing
-- course-hub's 20260918160000 revoke.
--
...
-- Put the rcs copy back to uncallable, as course-hub 20260918160000 left it.
revoke execute on function rcs.resolve_day_plan_payload(uuid) from public, anon, authenticated;

-- Belt and braces: clear any snapshot written since.
update public.day_plans
set published_payload = jsonb_set(
  published_payload,
  '{blocks}',
  (
    select coalesce(jsonb_agg(b - 'students' order by ord), '[]'::jsonb)
    from jsonb_array_elements(published_payload->'blocks') with ordinality as t(b, ord)
  )
)
where jsonb_typeof(published_payload->'blocks') = 'array'
  and exists (
    select 1 from jsonb_array_elements(published_payload->'blocks') as x(b)
    where x.b ? 'students'
  );
