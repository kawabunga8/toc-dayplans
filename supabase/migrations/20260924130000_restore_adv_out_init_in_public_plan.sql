-- Restore the adv_out initialisation in get_public_day_plan_live.
--
-- 20260924120000 rebuilt this function from the repo copy, which predated a
-- fix applied directly to the live database: without `adv_out := '{}'`,
-- adv_out starts NULL and every `adv_out || ...` stays NULL, so the public
-- plan silently dropped materials_needed, lesson_overview,
-- assessment_touch_points, pd_goal_connections and first_peoples_principles.
--
-- (The live copy also lacked `and visibility = 'link'`; that filter stays, per
-- ADR-0001/0003 — drafts must not be readable on the public page.)
