-- TOC Day Plans no longer carries any student data. The attendance roster was
-- removed from the public pages (8ea9079), but both payload functions still
-- attached each class's enrolled students (id, first/last name) to every
-- block, and get_public_day_plan_live / get_public_day_plan_by_id are
-- callable by anon — so names were reachable by calling the RPCs directly,
-- even though the app stripped them before rendering.
--
-- 1. Redefine both payload functions without the per-block `students` key.
-- 2. Strip `students` from every stored published_payload snapshot, which
--    get_public_day_plan_by_id returns as-is.
